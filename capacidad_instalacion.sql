...
BEGIN
  ...
  FOR rec1 IN SELECT t.day::date FROM generate_series(f_inicio, f_termino, interval '1 day') AS t(day) LOOP
    FOR rec2 IN SELECT DISTINCT * FROM dblink('host=localhost user=grupo85 dbname=grupo85e3 password=pieza312 port=5432', 'SELECT instalaciones.instalacion_id, puerto_id, capacidad FROM instalaciones') AS f1(instalacion_id int, puerto_id int, capacidad int) LOOP
      IF rec2.puerto_id = idpuerto THEN
        contador := 0;
        FOR rec3 IN SELECT DISTINCT * FROM dblink('host=localhost user=grupo85 dbname=grupo85e3 password=pieza312 port=5432', 'SELECT permiso_id, atraque, permisos.instalacion_id FROM permisos') AS f2(permiso_id int, atraque date, instalacion_id int) LOOP
          ...
          END IF;
        END LOOP;
        IF contador < rec2.capacidad AND rec2.puerto_id = idpuerto THEN
